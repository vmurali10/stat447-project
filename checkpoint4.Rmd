...
library(shiny)
library(tidyverse)
df <- read_csv("merged_data_full_join.csv")
...
forecast_heat <- function(state, column, forecast_year) {
  subdf <- df %>% filter(State == state) %>% arrange(Year)

  first_year <- min(subdf$Year, na.rm = TRUE)
  last_year  <- max(subdf$Year, na.rm = TRUE)
  steps      <- forecast_year - last_year

  # If the forecast year is within the existing data, return the actual value
  if (steps <= 0) {
    val <- subdf[[column]][subdf$Year == forecast_year]
    return(sprintf("In %s, the actual %s in %d was: %g", state, column, forecast_year, val))
  }

  ts_data <- ts(subdf[[column]], start = first_year, frequency = 1)
  if (all(is.na(ts_data))) return("No data available.")
  ts_data <- zoo::na.locf(ts_data, na.rm = FALSE)
  ...
  train_data <- window(ts_data, end = first_year + n_train - 1)
  test_data  <- window(ts_data, start = first_year + n_train)

  # Fit models
  model_nn    <- tryCatch(nnetar(train_data), error = function(e) NULL)
  model_lm    <- tryCatch(tslm(train_data ~ trend), error = function(e) NULL)
  model_ets   <- tryCatch(ets(train_data), error = function(e) NULL)
  model_arima <- tryCatch(auto.arima(train_data, seasonal = FALSE, allowdrift = TRUE), error = function(e) NULL)

  # Forecast test
  h <- length(test_data)
  fc_nn    <- tryCatch(forecast(model_nn, h = h)$mean, error = function(e) rep(NA, h))
  fc_lm    <- tryCatch(forecast(model_lm, h = h)$mean, error = function(e) rep(NA, h))
  fc_ets   <- tryCatch(forecast(model_ets, h = h)$mean, error = function(e) rep(NA, h))
  fc_arima <- tryCatch(forecast(model_arima, h = h)$mean, error = function(e) rep(NA, h))

  # Test MSE
  error_nn    <- mean((test_data - fc_nn)^2, na.rm = TRUE)
  error_lm    <- mean((test_data - fc_lm)^2, na.rm = TRUE)
  error_ets   <- mean((test_data - fc_ets)^2, na.rm = TRUE)
  error_arima <- mean((test_data - fc_arima)^2, na.rm = TRUE)

  errors <- c(NNETAR = error_nn, TSLM = error_lm, ETS = error_ets, ARIMA = error_arima)
  best_model_name <- names(which.min(errors))

  # Refit on full data
  model <- switch(best_model_name,
                  NNETAR = tryCatch(nnetar(ts_data), error = function(e) NULL),
                  TSLM   = tryCatch(tslm(ts_data ~ trend), error = function(e) NULL),
                  ETS    = tryCatch(ets(ts_data), error = function(e) NULL),
                  ARIMA  = tryCatch(auto.arima(ts_data, seasonal = FALSE, allowdrift = TRUE), error = function(e) NULL),
                  NULL)
  ...
  pred <- round(fc$mean[steps])
  pred <- max(pred, 0)

  return(sprintf(
    "Forecasted %s in %s for %d is: %d (Model: %s)",
    column, state, forecast_year, pred, best_model_name
  ))
}
...
library(zoo)
set.seed(123)
forecast_health <- function(state, variable, forecast_year) {
  subdf <- df %>% filter(State == state) %>% arrange(Year)
  ...
  if (steps <= 0) {
    val <- subdf[[variable]][subdf$Year == forecast_year]
    return(sprintf("In %s, the actual %s in %d was: %g", state, variable, forecast_year, val))
  }

  ts_data <- ts(subdf[[variable]], start = first_year, frequency = 1)
  if (all(is.na(ts_data))) return("No data available.")
  ts_data <- na.locf(ts_data, na.rm = FALSE)
  ...
  sprintf(
    "Forecasted %s in %s for %d is: %d (Model: %s)",
    variable, state, forecast_year, pred, best_model_name
  )
}
  ...
  tabsetPanel(
    ...
    tabPanel("Extreme Heat Days Forecast",
      sidebarLayout(
        ...
        mainPanel(
          width = 12,
            div(style = "background-color: #ffeaea; padding: 15px; border-radius: 10px;",
            uiOutput("result"),
            plotlyOutput("heat_plot_dy", height = "300px"),
            plotlyOutput("income_heat_plot_dy", height = "300px"),
            plotlyOutput("workerHRI_heat_plot_dy", height = "300px"),
            plotlyOutput("mortality_heat_plot_dy", height = "300px")
          )
        )
      )
    ),
      ...
      sidebarLayout(
        ...
        mainPanel(
          width = 12,
            ...
            plotlyOutput("health_plot_dy", height = "300px"),
            plotlyOutput("income_health_plot_dy", height = "300px"),
            plotlyOutput("workerHRI_health_plot_dy", height = "300px"),
            plotlyOutput("mortality_health_plot_dy", height = "300px")
          )
        )
      )
    )
...
observeEvent(input$go_health, {
  output$result_health <- renderUI({
    ...
    colname <- if (input$metric_health == "ER") {
      paste0("ER_visits_hri_age_", ifelse(input$age_health == "65+", "over_65", gsub("-", "_", input$age_health)))
    } else {
      paste0("HSP_visits_age_", ifelse(input$age_health == "65+", "over_65", gsub("-", "_", input$age_health)))
    }
    result <- forecast_health(input$state_health, colname, input$year_health)
...
output$heat_plot_dy <- renderPlotly({
  ...
  if (nrow(df_state) == 0 || all(is.na(df_state$value))) {
    return(ggplotly(
      ggplot() +
        theme_void() +
        annotate("text", x = 0.5, y = 0.5, label = "No available data for selection 😔", size = 6, hjust = 0.5)
    ))
  }

  last_real_year <- max(df_state$Year, na.rm = TRUE)
  ts_data <- ts(df_state$value, start = min(df_state$Year, na.rm = TRUE), frequency = 1)
  ts_data <- zoo::na.locf(ts_data, na.rm = FALSE)

  forecast_year <- 2030
  steps <- forecast_year - last_real_year
  ...
  if (steps > 0) {
    ...
    train_data <- window(ts_data, end = start(ts_data)[1] + n_train - 1)
    test_data  <- window(ts_data, start = start(ts_data)[1] + n_train)
    ...
    if (length(best_model_name) == 0 || is.na(best_model_name)) {
      return(ggplotly(
        ggplot() +
          theme_void() +
          annotate("text", x = 0.5, y = 0.5, label = "Model fitting failed 😔", size = 6, hjust = 0.5)
      ))
    }
    ...
    if (!is.null(model)) {
      fc <- forecast(model, h = steps)
      future_years <- (last_real_year + 1):(last_real_year + steps)
      forecast_df <- data.frame(Year = future_years, value = as.numeric(fc$mean))
    }
  }

  # Add Type for legend
  df_state$Type <- "Historical Data"
  if (!is.null(forecast_df)) {
    forecast_df$Type <- "6-Year Forecast (2024-2030)"
  }

  plot <- ggplot() +
    geom_line(data = df_state, aes(x = Year, y = value, color = Type), size = 1.2) +
    labs(
      title = paste("Heat Trend in", input$state, "for", input$column),
      y = input$column,
      x = "Year",
      color = "Legend"
    ) +
  ...
  if (!is.null(forecast_df)) {
    plot <- plot +
      geom_line(data = forecast_df, aes(x = Year, y = value, color = Type), size = 1.2, linetype = "dashed") +
      geom_point(data = forecast_df, aes(x = Year, y = value, color = Type), size = 2)
  }

  plot <- plot +
    scale_color_manual(values = c(
      "Historical Data" = "firebrick",
      "6-Year Forecast (2024-2030)" = "orange"
    ))
...
output$health_plot_dy <- renderPlotly({
  ...
  # 🚨 Check if no data available
  if (nrow(df_health) == 0 || all(is.na(df_health$value))) {
    return(ggplotly(
      ggplot() +
        theme_void() +
        annotate("text", x = 0.5, y = 0.5, label = "No available data for selection 😔", size = 6, hjust = 0.5)
  ...
  last_real_year <- max(df_health$Year, na.rm = TRUE)
  ts_data <- ts(df_health$value, start = min(df_health$Year, na.rm = TRUE), frequency = 1)
  ts_data <- zoo::na.locf(ts_data, na.rm = FALSE)
  ...
  if (steps > 0) {
    ...
    if (!is.null(model)) {
      fc <- forecast(model, h = steps)
      future_years <- (last_real_year + 1):(last_real_year + steps)
      future_df <- data.frame(Year = future_years, value = as.numeric(fc$mean))
    }
  }

  # Add Type for legend
  df_health$Type <- "Historical Data"
  if (!is.null(future_df)) {
    future_df$Type <- "6-Year Forecast (2024-2030)"
  }

  plot <- ggplot() +
    geom_line(data = df_health, aes(x = Year, y = value, color = Type), size = 1.2) +
    labs(
      title = paste(input$metric_health, "Trend in", input$state_health, "Age", input$age_health, "(forecasted to 2030)"),
  ...
  if (!is.null(future_df)) {
    plot <- plot +
      geom_line(data = future_df, aes(x = Year, y = value, color = Type), size = 1.2, linetype = "dashed") +
      geom_point(data = future_df, aes(x = Year, y = value, color = Type), size = 2)
  }
